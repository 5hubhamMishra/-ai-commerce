...
test.describe("browse, add to cart, and checkout", () => {
  ...
  test("removing the only item empties the cart and shows the empty state", async ({
    page,
  }) => {
    await registerAndSignIn(page);
  ...
  test("an authenticated shopper sees the empty-cart checkout state", async ({
    ...
    await expect(page.getByText("Your cart is empty")).toBeVisible();
    await expect(
      page.getByText("You cannot checkout with an empty cart."),
    ).toBeVisible();
  });
  ...
  test("a guest who tries to add to cart is sent to sign in, then back to the product", async ({
    ...
    await expect(page).toHaveURL(
      new RegExp(`/login\\?redirect=${encodeURIComponent(productPath)}`),
    );
