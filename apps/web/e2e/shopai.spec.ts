...
test.describe("ShopAI chat", () => {
  test("sending a message shows it immediately, then renders the real assistant reply", async ({
    page,
  }) => {
      ...
      route.fulfill({
        json: {
          ...
          message: {
            role: "assistant",
            content:
              "Here are a few laptops that fit a coding/ML workload under 80,000.",
          },
    ...
    await expect(
      page.getByText("a laptop for coding and machine learning under 80000"),
    ).toBeVisible();
    await expect(
      page.getByText(
        "Here are a few laptops that fit a coding/ML workload under 80,000.",
  ...
  test("a second message carries the conversation id the first reply returned", async ({
    page,
  }) => {
    let requestCount = 0;
  ...
  test("a failed request falls back to catalog guidance instead of hanging", async ({
    page,
  }) => {
    await page.route("**/shopai/message", (route) =>
      route.fulfill({
  ...
  test("a starter chip sends its own text as the first message", async ({
    page,
  }) => {
    await page.route("**/shopai/message", (route) =>
      route.fulfill({
        json: {
          conversationId: "e2e-conv-3",
          message: {
            role: "assistant",
    ...
    await page.goto("/ai-shopping");
    await page
      .getByRole("button", { name: /Running shoes under 10000/ })
      .click();

    await expect(
      page.getByText("Running shoes under 10000", { exact: false }),
    ).toBeVisible();
    await expect(page.getByText("Sure — here's what I found.")).toBeVisible({
      timeout: 5000,
    });
  });
