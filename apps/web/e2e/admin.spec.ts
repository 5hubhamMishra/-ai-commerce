...
test.describe("admin dashboard", () => {
  ...
  test("a signed-in customer (no admin role) sees a not-authorized message, not real data", async ({ page }) => {
    await registerAndSignIn(page);
    await page.goto("/admin");
    await expect(page.getByRole("heading", { name: "Not authorized" })).toBeVisible();
  });

  test("the seeded SUPER_ADMIN sees the real business dashboard with live data", async ({ page }) => {
    ...
    await expect(page.getByText(/\d+ profiles/)).toBeVisible({ timeout: 10000 });

    await expect(page.getByRole("heading", { name: "Recent Orders" })).toBeVisible();
  });

  test("the admin can open a real order and update its status", async ({ page }) => {
    await page.goto("/login");
    await page.getByLabel("Email address").fill("admin@veloura.dev");
    await page.getByLabel("Password").fill("ChangeMe123!");
    await page.getByRole("button", { name: "Sign in to Veloura" }).click();
    ...
    await expect(page.getByRole("button", { name: "Update status" })).toBeEnabled({ timeout: 10000 });
  });
});
