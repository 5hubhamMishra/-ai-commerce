...
-- NOT VALID lets this deployment succeed with legacy rows that need cleanup;
-- PostgreSQL still enforces each check on every new insert and update.
...
ALTER TABLE "inventory"
  ...
  CHECK ("quantity_reserved" + "quantity_committed" <= "quantity_on_hand") NOT VALID;
...
ALTER TABLE "order_items"
  ...
  CHECK ("unit_price" >= 0 AND "line_total" >= 0 AND "quantity" > 0) NOT VALID;
...
ALTER TABLE "sellers"
  ...
  CHECK ("commission_rate_bps" BETWEEN 0 AND 10000) NOT VALID;

ALTER TABLE "seller_earnings"
  ADD CONSTRAINT "seller_earnings_amounts_nonnegative_check"
  CHECK ("gross_amount" >= 0 AND "commission_amount" >= 0 AND "net_amount" >= 0) NOT VALID,
