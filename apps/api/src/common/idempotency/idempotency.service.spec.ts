...
describe('IdempotencyService', () => {
  ...
  it('claims the key, runs the operation once, and stores the response', async () => {
    ...
    const operation = jest.fn().mockResolvedValue({ statusCode: 201, body: { orderId: 'o1' } });

    const result = await service.run('user1', 'order_create', 'key1', operation);
    ...
    expect(prisma.idempotencyKey.update).toHaveBeenCalledWith({
      where: { userId_scope_key: { userId: 'user1', scope: 'order_create', key: 'key1' } },
      data: { statusCode: 201, responseBody: { orderId: 'o1' } },
    });
    expect(result).toEqual({ statusCode: 201, body: { orderId: 'o1' }, replayed: false });
  });

  it('replays the stored response instead of re-running a completed operation', async () => {
    prisma.idempotencyKey.create.mockRejectedValue(new Error('unique constraint violation'));
    ...
    expect(result).toEqual({ statusCode: 201, body: { orderId: 'o1' }, replayed: true });
  });
  ...
  it('releases the claim on failure so a genuine retry can re-attempt the operation', async () => {
    ...
    const operation = jest.fn().mockRejectedValue(new Error('payment provider unreachable'));
    ...
    expect(prisma.idempotencyKey.delete).toHaveBeenCalledWith({
      where: { userId_scope_key: { userId: 'user1', scope: 'payment_create', key: 'key1' } },
    });
