...
/** Fetches the entire real catalog once (paginated, cached for the app's lifetime) and returns
 *  an id -> product map. apps/api's recommendation endpoints return bare
...
export function useProductIndex(): Map<string, ProductListItem> | null {
  ...
  useEffect(() => {
    // Already captured correctly by useState's initializer above if it was set before this
    // component ever mounted — nothing to synchronize in that case.
